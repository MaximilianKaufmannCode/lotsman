...
"""Outbox dispatcher ARQ task for notification-service.

Polls notification.outbox WHERE dispatched_at IS NULL using FOR UPDATE SKIP LOCKED,
...
async def dispatch_outbox(ctx: dict[str, Any]) -> None:
    """ARQ task: poll notification.outbox and forward pending events to Redis Streams."""
