...
**Лоцман убирает эту боль.** Он хранит документы по компаниям, **сам напоминает** ответственным о приближении сроков — за N дней, в день срока и повторно при просрочке, — фиксирует **каждое** изменение и работает **на вашем сервере**. Источник правды — база данных, а не файл.
...
| Нет разграничения доступа | Роли: Администратор · Редактор · Читатель + обязательный 2FA |
| Данные в чужом облаке | Self-hosted: данные не покидают ваш контур |

## Что умеет

- 📋 **Привычно, как Excel.** Виртуализированная таблица: закреплённые шапки, цветные маркеры сроков (🟢 ок · 🟠 скоро · 🔴 просрочено), правка по двойному клику, быстрый поиск. Учиться нечему.
...
- 📨 **Доставка туда, где люди.** Письма приходят брендированным HTML — с цветом срочности, прямой кнопкой «Открыть документ» и человеческими датами; каналы: Email (SMTP/EWS), Telegram, Dion.
...
- ➕ **Быстрый ввод без отвлечения.** Компании и типы документов можно завести прямо из формы добавления документа — без ухода со страницы и потери заполненного.
- 🔠 **Под себя.** Размер шрифта интерфейса настраивается [в профиле](docs/user-guide/profile/font-size.md) (4 пресета, совместимо с зумом браузера).

## Кому подойдёт

Небольшим командам (≈2–10 человек) — юристам, комплаенсу, АХО, закупкам — которые ведут реестр договоров / лицензий / аудиторских отчётов по компаниям и хотят уйти от Excel и ручных напоминаний, **не отдавая данные в облако**.
...
Четыре доменных сервиса на FastAPI (auth · registry · notification · audit) + шлюз **web-bff**, SPA на React 19, PostgreSQL и Redis — всё в Docker Compose, on-premise. Чистая архитектура внутри каждого сервиса, звёздная топология через BFF, транзакционный outbox → Redis Streams → аудит. (В dev-окружении есть ещё привилегированный сайдкар **system-control** для операций обслуживания — в проде он отсутствует.)
...
Лоцман распространяется под [Business Source License 1.1](LICENSE) (BUSL-1.1) — лицензией source-available. Использование, включая внутреннее в организации, разрешено; предоставление Лоцмана третьим лицам как конкурирующего сервиса или продукта не допускается до наступления Change Date, по которой каждая версия переходит под MPL 2.0. Полное изложение условий — [LICENSING.md](LICENSING.md).
