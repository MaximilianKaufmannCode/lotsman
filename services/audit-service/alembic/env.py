...
"""Alembic env.py — audit-service.

Online mode uses asyncpg (async engine).
Offline mode uses psycopg (sync) for plain SQL generation without a live DB.

Database URL is read from AUDIT_DATABASE_URL environment variable.
Example:
    postgresql+asyncpg://audit_app:secret@localhost:5432/lotsman
"""
...
from audit_service.db.models import Base  # noqa: E402

target_metadata = Base.metadata

_raw_url = os.environ.get("AUDIT_DATABASE_URL")
if not _raw_url:
    raise RuntimeError(
        "AUDIT_DATABASE_URL environment variable is not set. "
        "Set it before running alembic commands:\n"
        "  export AUDIT_DATABASE_URL=postgresql+asyncpg://audit_app:secret@localhost:5432/lotsman"
    )
...
def include_object(object, name, type_, reflected, compare_to):  # type: ignore[override]
    if type_ == "table":
        return getattr(object, "schema", None) == "audit"
    return True
